# Robust bootstrap: no here-strings; writes each file from an array of lines.
# Requires VS 2022 C++ workload + CMake 3.21+
...
'CMakeFiles/',
'CMakeCache.txt',
'*.obj','*.o','*.pdb','*.exe','*.dll','*.lib','*.exp','*.ilk','*.log','*.user','*.VC.db','*.DS_Store','*.ipch',
'.vscode/','.idea/'
)

# CMakeLists.txt
Write-Lines "CMakeLists.txt" @(
...
Write-Lines "include/atmsp/logging.h" @(
'#pragma once',
'#include <memory>',
'#include <string>',
'#include <vector>',
...
Write-Lines "include/atmsp/events.h" @(
'#pragma once',
'#include <string>',
'#include <variant>',
'#include <chrono>',
...
Write-Lines "include/atmsp/event_bus.h" @(
'#pragma once',
'#include <functional>',
'#include <mutex>',
'#include <vector>',
...
Write-Lines "include/atmsp/errors.h" @(
'#pragma once',
'#include <string_view>',
'',
'namespace atmsp {',
...
Write-Lines "include/atmsp/sp_interface.h" @(
'#pragma once',
'#include <string>',
'#include <future>',
...
Write-Lines "include/atmsp/session.h" @(
'#pragma once',
'#include <string>',
'#include <nlohmann/json.hpp>',
'#include "event_bus.h"',
...
# Device SP headers (used by mocks)
Write-Lines "include/atmsp/card_reader_sp.h" @(
'#pragma once',
...
Write-Lines "include/atmsp/pin_pad_sp.h" @(
'#pragma once',
'#include "sp_interface.h"',
'namespace atmsp {',
...
# ===== src =====
Write-Lines "src/logging.cpp" @('#include "atmsp/logging.h"')
Write-Lines "src/session.cpp" @('#include "atmsp/session.h"')

Write-Lines "src/mock_card_reader.cpp" @(
'#include <thread>',
'#include <chrono>',
'#include <random>',
'#include <atomic>',
...
Write-Lines "src/mock_pin_pad.cpp" @(
'#include <thread>',
'#include <chrono>',
'#include <atomic>',
...
# ===== tests =====
Write-Lines "tests/test_event_bus.cpp" @(
...
Write-Lines "tests/test_session.cpp" @(
'#include <gtest/gtest.h>',
'#include "atmsp/session.h"',
'',
'using namespace atmsp;',
...
Write-Lines "tests/test_mocks.cpp" @(
'#include <gtest/gtest.h>',
'#include <chrono>',
'#include <thread>',
'#include "atmsp/card_reader_sp.h"',
...
Write-Host '✅ Minimal project scaffold created.'
